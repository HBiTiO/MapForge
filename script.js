...
function renderProjects(){const list=filter==="Tous"?PROJECTS:PROJECTS.filter(p=>p.category===filter);empty.classList.toggle("hidden",!!list.length);projectsBox.innerHTML=list.map(p=>`<article class="project reveal"><button class="cover" type="button" data-open="${esc(p.id)}">${p.image?`<img src="${esc(p.image)}" alt="${esc(p.title)}" loading="lazy">`:`<span>IMAGE DU PROJET</span>`}<i>Voir le projet ↗</i></button><div class="project-body"><div class="tags">${(p.tags||[p.category]).map(t=>`<span>${esc(t)}</span>`).join("")}</div><div class="project-head"><h3>${esc(p.title)}</h3><strong>${esc(p.price||"")}</strong></div><p>${esc(p.description)}</p><button class="discover" type="button" data-open="${esc(p.id)}">Découvrir <span>↗</span></button></div></article>`).join("");$$('[data-open]',projectsBox).forEach(b=>b.onclick=()=>openProject(b.dataset.open));$("#project-count").textContent=PROJECTS.length;observe()}
...
$("#copy-discord").onclick=async()=>{try{await navigator.clipboard.writeText(SITE_CONFIG.discord);$("#copy-message").classList.add("show");setTimeout(()=>$("#copy-message").classList.remove("show"),1800)}catch{prompt("Copie ton pseudo Discord :",SITE_CONFIG.discord)}};
$("#contact-email").textContent=SITE_CONFIG.email;$("#contact-email-link").href=`mailto:${SITE_CONFIG.email}`;$("#contact-discord-name").textContent=SITE_CONFIG.discordDisplay||SITE_CONFIG.discord;$("#contact-discord-handle").textContent=`@${SITE_CONFIG.discord}`;
const menu=$("#menu-btn"),nav=$("#main-nav");menu.onclick=()=>{const open=nav.classList.toggle("open");menu.classList.toggle("open",open);menu.setAttribute("aria-expanded",open)};$$('a',nav).forEach(a=>a.onclick=()=>{nav.classList.remove("open");menu.classList.remove("open");menu.setAttribute("aria-expanded","false")});
let observer;function observe(){if(!observer)observer=new IntersectionObserver(es=>es.forEach(e=>{if(e.isIntersecting){e.target.classList.add("show");observer.unobserve(e.target)}}),{threshold:.1});$$('.reveal').forEach(e=>observer.observe(e))}
renderFilters();renderProjects();observe();
